...
## 추가 검토 — vrf 안에 fake nexthop reachability 미리 넣기 (시도 예정)

위에서 막힌 핵심은 ovn-controller가 install한 route의 nexthop 172.16.1.113이 vrf-evpn 안에서 reachable하지 않다는 점. 그런데 data plane은 어차피 OVN의 default VRF 경로가 처리하니 vrf-evpn 안에서 패킷이 실제 forwarding될 필요가 없다. **광고만 통과시키면 되는 거라면 fake reachability stub만 미리 깔아둬도 동작할 가능성이 있다.**

세 가지 시도 후보:

**A. FRR zebra에 cross-VRF nexthop resolve 허용**
...
가장 단순. 한 줄로 끝. zebra가 vrf-evpn 안의 nexthop을 default VRF 통해 resolve. 대신 multi-tenant 환경에서 VRF 격리가 약해진다는 점이 마음에 걸린다.

**B. vrf-evpn 안에 services 망 static route 추가**

```bash
ip route add 172.16.1.0/24 dev br-evpn vrf vrf-evpn proto static
```

br-evpn으로 가는 길이라 패킷이 실제로 forwarding되면 VXLAN encap에서 dst 없어 drop. 광고만 하는 경우엔 OK. zebra가 이걸 통해 nexthop 172.16.1.113을 resolve할 수 있을지가 관건.

**C. dummy device로 fake connected route**

```bash
ip link add svc-stub type dummy
ip link set svc-stub master vrf-evpn
ip link set svc-stub up
ip addr add 172.16.1.250/24 dev svc-stub   # services subnet의 미사용 IP
```

vrf-evpn에 `172.16.1.0/24 dev svc-stub proto kernel scope link` connected route 자동 생성. 의미적으로 가장 깨끗. dummy라 ARP 안 함, services L2 충돌 없음.

### 시도 순서

A → 동작 안 하거나 multi-tenant 우려가 있으면 C.

### 한계

- ovn-controller가 install하는 route의 metric/dev에 따라 NHT가 어떻게 판정할지 실측 필요
- `local-only=true` 토글 시 광고 사라지는 race는 별개 문제 (nexthop과 무관)
- 본질적인 해결이 아니라 광고만 통과시키는 trick. spec의 dummy LS + Agent FSM 통합과는 다른 접근
- production에선 ToR fabric으로 가는 G1이 더 안전. 이 trick은 ToR 인프라 전 임시 또는 보조 경로 용

### TODO

- [ ] 각 옵션 실측. 어디까지 active 되고 어떤 prefix가 광고되는지 기록
- [ ] 결과 정리 후 02-why-it-failed.md의 nexthop 논의에도 후속 노트 추가
