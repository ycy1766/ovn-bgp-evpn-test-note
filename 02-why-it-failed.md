...
광고만 하는 거면 vrf 안의 nexthop reachability를 fake로 미리 만들어두는 trick으로 우회될 수도 있다 (`ip nht resolve-via-default`, dummy device로 가짜 connected route 등). 01의 "추가 검토" 섹션에 옵션 정리해두고 시도 예정. 동작하면 ToR 인프라 도입 전 임시로 compute 직접 종단도 가능해진다. 단 본질적인 해결은 아니고, spec의 dummy LS + Agent FSM 통합과는 다른 hack 성격이라 production 권장은 여전히 G1.
