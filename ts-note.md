...
## 7. 운영 점검 명령 (live verification runbook)

본 incident 의 근본원인 검증, 다른 환경의 동일 취약 구조 여부 확인, 진단·즉시조치까지 — 이 세션에서 실제로 사용한 명령을 한 묶음으로 정리.

### 7.1 환경 준비 — RabbitMQ pod 자동 탐색
환경마다 Pod 이름이 다를 수 있어 라벨로 탐색. NS 만 바꾸면 다른 리전에서도 그대로 동작.
```bash
NS=openstack
RMQ=$(kubectl get pods -n $NS -l app.kubernetes.io/component=rabbitmq -o name | head -1)
echo "rabbitmq pod: $RMQ"
# 출력 예: pod/rabbitmq-server-0
```

### 7.2 reply queue 속성 확인 (★ 근본원인 결정적 검증)
oslo 가 reply queue 를 어떻게 선언했는지 직접 본다. § 2.7 의 결정적 증거를 만든 명령.
```bash
kubectl exec -n $NS $RMQ -c rabbitmq -- \
  rabbitmqctl list_queues -p nova name type exclusive auto_delete durable policy arguments --no-table-headers \
  | grep -E '^reply_' | head -5
```
이번 환경 실측 출력:
```
reply_42ba8f8c3c054ffcaccb9b13cef65f66  classic false  false  false        [{"x-expires",1800000},{"x-queue-type","classic"}]
                                        ↑type   ↑excl  ↑auto  ↑durable ↑policy=∅  ↑arguments(oslo 가 선언)
```
**해석:**
- `type=classic` · `exclusive=false` · `auto_delete=false` → § 2.2 의 "독립 수명" 큐 확정
- `policy` 컬럼이 **비어 있음** → `nova-quorum-three-replicas`(.*) 가 reply queue 에 **붙지도 않음** → 원 보고서의 policy 귀속이 틀렸다는 결정적 증거
- `arguments` 의 `x-expires=1800000`, `x-queue-type=classic` 은 전부 **oslo 가 클라이언트로 선언한 것** (vhost·policy 가 아니라)

### 7.3 vhost 의 policy 정의 확인 (policy 가 어떻게 정의돼 있는지)
```bash
kubectl exec -n $NS $RMQ -c rabbitmq -- rabbitmqctl list_policies -p nova
```
이번 환경 실측 출력:
```
nova  nova-quorum-three-replicas  .*  queues  {"target-group-size":3}  0
```
**해석:** definition 이 `target-group-size: 3` **뿐** (quorum 큐 전용 파라미터). expires/exclusive 가 없음 → policy 는 reply queue 동작에 영향을 줄 수 없음. § 7.2 의 결과와 합쳐 인과 confirm.

### 7.4 단일 vhost 좀비 카운트 (가장 빠른 한 줄)
```bash
V=nova
kubectl exec -n $NS $RMQ -c rabbitmq -- \
  rabbitmqctl list_queues -p $V name consumers --no-table-headers \
  | awk '$1 ~ /^reply_/ && $2==0' | wc -l
# 출력 예: 1760
```
**해석:** consumer=0 인 reply 큐 개수. 1,760 = 본 incident 시점 관측치. 양성(probe 부산물, § 2.3) 이지만 **개수 자체가 "동일 취약 구조"의 지표.**

### 7.5 전 vhost 좀비 스캔 (★ 다른 환경 구조 동일성 확인)
다른 리전·클러스터에 같은 이슈가 잠재해 있는지 한 번에 본다.
```bash
NS=openstack
RMQ=$(kubectl get pods -n $NS -l app.kubernetes.io/component=rabbitmq -o name | head -1)
...
이번 환경 실측 출력 (요약):
```
nova        reply_total=1892   zombie(consumer=0)=1760    ← compute 노드 수 × probe 빈도
cinder      reply_total=27     zombie(consumer=0)=0
manila      reply_total=23     zombie(consumer=0)=0
neutron     reply_total=0      zombie(consumer=0)=0
...
```
**해석 가이드:**
- 어떤 vhost 든 `zombie > 0` → 그 환경도 **동일 취약 구조** (= 동일 root cause 노출)
- `zombie = 0` 이어도 *구조* 는 같음 — 단지 활동량(probe·RPC 빈도)이 적어 안 보일 뿐
- nova 가 압도적인 건 자연스러움 (§ 2.8 — health-probe × 노드 수)

### 7.6 좀비 큐 상세 (메모리 점유 큰 순)
부하 누적이 의심될 때 무엇이 메모리를 잡고 있는지.
```bash
V=nova
# 메모리 큰 순 top 20
kubectl exec -n $NS $RMQ -c rabbitmq -- \
  rabbitmqctl list_queues -p "$V" name consumers messages memory --no-table-headers \
  | awk '$2==0 && $1 ~ /^reply_/' | sort -k4 -nr | head -20

# 합계 (개수 + 누적 메모리 MB)
kubectl exec -n $NS $RMQ -c rabbitmq -- \
  rabbitmqctl list_queues -p "$V" name consumers memory --no-table-headers \
  | awk '$1 ~ /^reply_/ && $2==0 {n++; m+=$3} END {printf "zombie=%d  mem=%.1fMB\n", n, m/1048576}'
```

### 7.7 oslo.messaging 버전 확인 (4종 세트 적용 가능 여부)
Bug #2110957 fix(`d3a4eedb8a14`) 포함 여부 판정의 출발점.
```bash
NS=openstack
POD=$(kubectl get pods -n $NS -l application=nova,component=conductor -o name | head -1)
kubectl exec -n $NS $POD -- pip show oslo.messaging | grep -iE '^Version'
# 출력 예: Version: 16.1.0
```
**해석:** 16.1.0 이면 fix 포함 (§ 5.3 확인). 16.1.0 미만이거나 14.x 라면 다른 길이 필요.

### 7.8 fix 커밋 포함 여부 결정적 검증 (GitHub 미러)
버전 숫자만으론 모호할 때, 실제 커밋이 태그에 포함됐는지 본다.
```bash
gh api repos/openstack/oslo.messaging/compare/16.1.0...d3a4eedb8a14 \
  --jq '{status:.status, ahead_by:.ahead_by, behind_by:.behind_by}'
# 출력 예: {"status":"behind","ahead_by":0,"behind_by":4}
```
**해석:** `ahead_by=0` ⇒ fix 커밋이 16.1.0 의 **조상**(= 포함됨).

### 7.9 빌드 행 진단 — 한 인스턴스가 BUILD 정지일 때
어떤 인스턴스가 이 이슈로 멈춰 있는지 1~3분에 결정.
```bash
UUID=<인스턴스 UUID>
NS=openstack

# (a) target compute 식별
openstack server show $UUID -f value \
  -c OS-EXT-SRV-ATTR:host -c status -c OS-EXT-STS:task_state
# 기대: host=<compute hostname>, status=BUILD, task_state=None

# (b) 그 compute 의 RPC 마비 여부 (이 이슈의 지문 ①)
HOST=<위에서 나온 host>
POD=$(kubectl get pods -n $NS -l application=nova,component=compute \
       --field-selector spec.nodeName=$HOST -o name | head -1)
kubectl logs -n $NS $POD -c nova-compute --tail=300 \
  | grep -iE 'MessagingTimeout|Timed out waiting for a reply'

# (c) conductor 가 그 노드 reply 큐로 못 보내는지 (지문 ②)
kubectl logs -n $NS -l application=nova,component=conductor --tail=500 \
  | grep -iE 'MessageUndeliverable|missing queue|Abandoning|doesn.t exist, drop reply'

# (d) 살아있지만 consumer=0 인 reply 큐 직접 식별
RMQ=$(kubectl get pods -n $NS -l app.kubernetes.io/component=rabbitmq -o name | head -1)
kubectl exec -n $NS $RMQ -c rabbitmq -- \
  rabbitmqctl list_queues -p nova name consumers --no-table-headers \
  | awk '$1 ~ /^reply_/ && $2==0'
```
**판정:** (b) 와 (c) 가 같은 host 에 대해 동시 발생 → **이 incident 확정.** 깨끗하면 다른 원인(neutron port binding / cinder / glance / placement).

### 7.10 즉시조치 — 영구 마비된 compute Pod 재시작
```bash
kubectl delete pod -n $NS $POD
# = 새 connection + 새 reply 큐 생성 → 즉시 복구 (보고서의 즉시조치 그대로)
```
**근본 해결은 아님** — 4종 세트 적용 전까지는 트리거 조건이 다시 모이면 재발 가능.

### 7.11 명령 모음 — 한눈 요약
| § | 무엇 확인 | 핵심 명령 |
|---|---|---|
| 7.2 | reply queue 속성·policy 미부착 | `rabbitmqctl list_queues -p nova name type exclusive auto_delete durable policy arguments` |
| 7.3 | vhost policy 정의 | `rabbitmqctl list_policies -p nova` |
| 7.4 | 단일 vhost 좀비 카운트 | `list_queues ... \| awk '$1~/^reply_/&&$2==0' \| wc -l` |
| 7.5 | 전 vhost 좀비 스캔 | for V in $(list_vhosts); do ... |
| 7.6 | 좀비 메모리 점유 | `list_queues ... memory` + sort |
| 7.7 | oslo 버전 | `pip show oslo.messaging` |
| 7.8 | fix 포함 검증 | `gh api compare/<tag>...<sha>` |
| 7.9 | 빌드 행 진단 | logs grep + consumer=0 |
| 7.10 | 즉시조치 | `kubectl delete pod` |
...
## 부록 B — 쉽게 풀어서 (비유로 보는 한 페이지)

> 기술 용어 없이, 이번 일이 정확히 무슨 일이었는지.

### 한 문장
**"답장 받을 내 우편함이 조용히 폐기됐는데, 정작 본인은 그걸 모르고 그 우편함으로 답장을 받으려 영원히 기다렸다."**

### 등장인물 (비유)
- **nova-compute** = 일하는 직원(각 compute 노드)
- **conductor** = 직원의 요청을 처리해주는 본사 사무직
- **reply queue** = 직원 책상 위 **개인 우편함** (답장 받는 곳)
- **RabbitMQ** = 우체국

### 평소엔 이렇게 돕니다
직원이 conductor 에게 "이거 처리해서 **내 우편함(reply_xxx) 으로 답장 줘**" 보냄 → conductor 처리 → 그 우편함에 답장 넣어줌 → 직원이 답장 보고 일 계속.

### 그런데 우체국에는 이런 규칙이 있습니다
**"우편함을 30분 동안 아무도 안 비우면 자동 폐기."**
- 평소엔 직원이 1분마다 우편함 비우니까(주기 작업) **30분 타이머가 시작도 안 함.**
- 그런데 직원에게 사소한 사고(잠깐의 IO 멈춤·네트워크 깜빡임 등)가 생겨 30분 동안 우편함을 못 비웠음.
- 시간 만료 → 우체국이 **규칙대로** 우편함 폐기.
- 직원은 자기 우편함이 사라진 줄 **모름** — 다음 요청에서도 같은 주소를 적어 보냄.
- conductor 가 답장하려는데 **그 우편함이 없음** → "수취인 불명"으로 반송. 로그에 `MessageUndeliverable`.
- 직원은 답장을 영원히 못 받음 → 모든 일이 60초 timeout. **재출근(Pod 재시작) 전까지 자가복구 안 됨.**

### 그래서 인스턴스 빌드가 멈춥니다
- "이 인스턴스 만들어"라는 **지시는 답장 필요 없는 단방향**이라 빌드 시작은 됨.
- 그런데 빌드 도중 "상태 저장해줘", "디스크 정보 줘" 같은 **답장 필요한** 요청이 많음.
- 우편함이 죽어 있어서 그 답장들이 다 안 옴 → 빌드가 중간에서 멈춤. API 에선 `host=None, building` 으로만 보임 → **빌드 행(hang).**

### "좀비 우편함"은 뭐였나
헬스체크 같은 짧은 프로세스도 우편함을 만들고 자기 답장 받고 종료함. 그런데 위 규칙 때문에 **종료해도 우편함은 30분 더 남아 있다가 폐기.** 그래서 빈 우편함이 항상 어느 정도 떠다님(관측 1,760개).
- **이건 양성.** 주인이 이미 떠났으니 답장 기다리는 사람도 없음. 청소되면 끝.
- 장애와 **청소 규칙은 똑같다.** 차이는 단 하나 — **장애는 주인이 살아 있는데 우편함이 폐기되는** 경우.

### 왜 5개월 잘 쓰다 이제야?
- "내 우편함이 30분 연속으로 안 비워진다"는 사건 자체가 **드물게만** 일어남.
- 초기엔 잦은 배포로 직원들이 자주 재출근 → 사고가 나도 새 우편함으로 갈리며 조용히 리셋됐을 것.
- 회사가 안정화돼서 **재출근이 뜸해진 시점** + **사소한 사고가 그 노드에서 처음 30분 이상 지속된 시점**이 5월에 겹친 것.
- "갑자기 망가졌다"가 아니라 "충분히 오래 굴리면 언젠가 한 번은 그 조건이 정렬되는 게 자연스러운" 드문 정렬.

### 그래서 어떻게 고치나 — 우편함 모델을 바꾼다
1. **3대의 우체국이 복제 보관하는 튼튼한 우편함으로** (`rabbit_transient_quorum_queue: true`) → 한 우체국 사고에도 우편함 안 사라짐.
2. **각 직원에 전담 우편함 매니저** (`use_queue_manager: true`) → 일정한 주소의 우편함을 매니저가 끝까지 관리. 사고 나도 매니저가 다시 세팅. 매번 새 주소 안 만들어도 됨(좀비도 멈춤).
3. **단체 공지용 큐는 다른 모델로** (`rabbit_stream_fanout: true`).
4. **기존 옵션 유지** (`rabbit_quorum_queue: true`).
→ 이 4개를 함께 켜는 게 OpenStack 공식 권장 조합. **하나만 켜면 검증된 조합이 아님.**

### "근데 이거 켜도 진짜 괜찮나?" 걱정 해소
- 이 4개를 함께 켜면 또 다른 문제(매니저+quorum 환경에서 답장이 엉뚱한 곳으로 가는 #2110957)가 있었음.
- 그 fix 가 **지금 우리가 깔아둔 oslo.messaging 16.1.0 에 이미 들어가 있다는 걸 미러에서 확인했음.**
- 길은 열려 있음. 단 부하 상황까지 **dev 에서 검증 후 canary 롤아웃.**

### 한 줄로
**"우편함이 사라져도 본인이 알아채고 새로 만들도록 모델 자체를 바꾸는 게 이 일의 끝이다."**

---

## 부록 C — probe 시각의 재검토 (왜 probe 수정만으론 부족한가)

> 자연스러운 의문: "좀비가 1,760개나 쌓이는 게 health-probe 때문이면, probe 만 고치면 되지 않나?" → **probe 수정은 좀비/브로커 부하는 해결하지만 이번 장애(노드 마비)는 못 막는다.** 원 보고서가 RabbitMQ policy 를 원인으로 본 것과 비슷한 함정 — 좀비 원인과 장애 원인이 같은 부모(oslo transient 큐 모델)를 공유하지만 **서로 다른 자식**이다.

### C.1 probe 가 실제로 하는 일 (current master 동작)
파일: [`nova/templates/bin/_health-probe.py.tpl`](https://github.com/openstack/openstack-helm/blob/master/nova/templates/bin/_health-probe.py.tpl) 핵심 흐름:
```python
# kubelet 이 liveness/readiness 마다 (≈90초 주기) 새 Python 프로세스로 실행
transport = oslo_messaging.get_rpc_transport(cfg.CONF)   # ① 매 invocation 새 transport
                                                         #    → 새 connection + 새 reply_<uuid> 큐
target = oslo_messaging.Target(
    topic='compute',                                     # service-queue-name
    server=hostname,                                     # 자기 자신
    namespace='baseapi', version='1.1')
client = oslo_messaging.get_rpc_client(transport, target, timeout=60, retry=2)
client.call(ctxt, 'ping', arg=None)                      # ② compute 의 ping 엔드포인트로 RPC call
                                                         #    응답이 오면 ping 성공 → exit 0
# 예외 처리:
#   MessageDeliveryFailure → broker 도달 불가 → exit 0 (성공으로 처리, broker는 별개 관심사)
#   RemoteError (Endpoint not support) → exit 0 (call 은 도달함)
#   MessagingTimeout → exit 1 (실패)
finally:
    transport.cleanup()                                  # ③ 2025-03-12 추가 — 단순 connection 정리
```
**큐 lifecycle:**
- ① 단계에서 `reply_<uuid>` 큐 생성(`exclusive=false · auto_delete=false · x-expires=30m`).
- 프로세스 종료 시 ③ 의 cleanup 은 **AMQP connection 만 정리**하지 **큐는 안 지움** (`exclusive=false`라 connection close 와 큐 lifecycle 이 분리). → 큐는 30분 동안 좀비로 남았다가 TTL 로 청소.

좀비 모집단 추정: probe 빈도 × TTL × Pod 수 ≈ 관측 1,760개와 부합.

### C.2 probe vs 이번 장애 — 관계 분리
- 죽은 큐 = `reply_2facd0a5...`, 주인 = nova-compute **PID 22555** (장기 데몬, probe 아님). probe 가 만든 큐와 compute 메인 reply 큐는 **완전히 다른 큐.**
- probe 를 꺼도 compute 의 메인 reply 큐가 consumer=0 30분 조건을 맞으면 **동일하게 마비.**
- probe 가 기여하는 건 (a) 좀비 1,760개 (양성), (b) broker 채널 활동 증가로 다른 큐의 채널 이벤트 확률을 *약간* 높임 (간접).

### C.3 probe 가 장애를 detect/prevent 할 수 있는가
**검사 방향이 어긋난다:**
```
[probe 가 검사하는 방향]      probe ──ping──▶ compute ──reply──▶ probe 의 임시 큐
[이번 장애가 발생한 방향]    compute ──call──▶ conductor ──reply──▶ compute 메인 큐 (= 죽은 큐)
                              ↑ 이 방향은 probe 가 안 봄
```
→ **현재 probe 는 장애 중에도 정상 통과.** ("service report 가 fanout 기반이라 conductor 엔 up 으로 보임"이라는 보고서 §2.1 의 침묵성과 동일 원리.)
- **detect**: probe 를 반대 방향(compute → 자기 메인 reply 큐 왕복) 으로 재설계하면 detect 가능. 단 **prevent 는 별개** — 검출 후 누가 Pod 를 재시작해야 함.
- **prevent**: probe 가 할 수 없음. 구조 자체(연결과 분리된 휘발성 큐 + oslo 자가복구 부재)는 4종 세트만이 닫는다.

### C.4 upstream 이력 — "재설계 없음"
| 시점 | 커밋 | 무엇 |
|---|---|---|
| 2019-03 | [`49b58b7e`](https://github.com/openstack/openstack-helm/commit/49b58b7e7d51) | 최초 Health probe for Nova components |
| 2019-05 | [`baf5356a`](https://github.com/openstack/openstack-helm/commit/baf5356a4fb6) | "Use nova's ping method ..." — **현재 RPC 핑 모델 정립** |
| 2019-06 | [`9bcf0df9`](https://github.com/openstack/openstack-helm/commit/9bcf0df94c1a) | "directly hit RabbitMQ servers" (DNS 우회) |
| 2020-02 | [`414b10fa`](https://github.com/openstack/openstack-helm/commit/414b10fab073) | concurrency/timings 정리 |
| 2023-05 | [`969e3f78`](https://github.com/openstack/openstack-helm/commit/969e3f78712f) | RPCClient API 갱신 |
| **2025-03** | **[`09e2d55c`](https://github.com/openstack/openstack-helm/commit/09e2d55cc41a)** | **"cleanly close connection" — broker disconnect log 노이즈만 해소, 큐 양산 구조는 그대로** |
| 2026-04 | [`c69d3ce5`](https://github.com/openstack/openstack-helm/commit/c69d3ce5e08b) | config parsing 개선 |

→ **2019년에 자리잡은 "매 probe 마다 새 RPC transport" 모델이 7년째 그대로.** zombie-queue 양산을 인지해 재설계한 흔적 없음. 2025-03 패치도 큐 lifecycle 은 안 건드림.

### C.5 ★ 4종 세트와 probe 의 상호작용 — 반드시 체크할 함정
`rabbit_transient_quorum_queue: true` 를 켜면 reply 큐가 **classic → quorum** 으로 바뀐다. quorum 큐는 classic 보다 declare/replicate 비용이 훨씬 비싸다(3노드 Raft 합의).

현재 probe 는 매 invocation 마다 **새 pid → 새 reply 큐**. queue manager 는 같은 프로세스 내 큐 풀을 관리할 뿐, **각 probe 가 fresh pid 라 manager 가 reuse 불가** → 결과적으로 **probe-induced quorum 큐 churn 이 새로 생김.**

> ⚠️ **즉 probe 를 안 고치고 4종 세트만 켜면, 좀비가 classic 에서 quorum 으로 바뀌어 broker 부하가 오히려 늘 수 있다.** dev 부하 테스트에서 반드시 측정.
>
> 측정 지표: 시간당 transient quorum 큐 생성/만료 수, RabbitMQ 메모리, raft log churn, queue declare latency.

### C.6 probe 재설계 옵션 (장기 개선 — 4종 세트 적용 후 검토)
| 안 | 효과 | 단점 |
|---|---|---|
| **(a)** AMQP 안 쓰는 local liveness (PID/소켓만) | 좀비 0, broker 부하 0 | broker 도달성 미검증 → 본 incident 같은 케이스 못 잡음 |
| **(b)** sidecar 가 reply 큐 영구 보유, kubelet 은 sidecar 에 HTTP probe | 좀비 0, broker 부하 최소, 검증 깊이 유지 | 차트 구조 변경 큼 (가장 정공법) |
| **(c)** `client.cast` (응답 없음) 로 변경 | 좀비 0 (reply 큐 안 만듦) | "응답 받는 것"을 못 봐 검증 깊이↓ |
| **(d)** 현행 + reply 큐 재사용 (process pool) | 좀비↓ | Pod 단위 sidecar 필요 — 결국 (b) 로 수렴 |

(b) 가 정공법인데 — **upstream 이 7년간 안 했다는 사실은 합의 어렵거나 우선순위 낮음을 시사.** 우리가 자체 patch 를 들고 가야 함.

### C.7 정직한 우선순위
| # | 작업 | 이유 |
|---|---|---|
| **1** | **4종 세트 적용 (canary → 전체)** | 이번 장애를 막는 것은 **오직 이것뿐**. probe 와 무관하게 구조적 결함을 닫음 |
| **2** | **dev 에서 probe-induced quorum 큐 churn 측정** | 4종 세트 후 RabbitMQ 부하 변화 확인. 견딜 만하면 probe 보류, 임계점이면 3 진행 |
| **3** | **probe 재설계 (b 안: sidecar 모델)** | upstream patch 추진 또는 downstream override. **장기 개선**, 이번 장애 즉시 대응과는 분리 |
| **4** | (옵션) probe 를 reverse 방향(compute → conductor 왕복) 로 보강 | detect 강화. prevent 는 여전히 4종 세트가 함 |

**probe 수정만으로 가는 시나리오(4종 세트 보류)는 권장 안 함** — 이번 장애 재발을 막지 못한다.
